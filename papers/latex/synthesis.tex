...
compositional grammar, and gauge/fault-tolerant high availability.  We
...
composition} that we track explicitly at each level.
...
absent from any single factor.  Composition is what produces these emergent
properties, and it does so without collapsing the six modules into a single primitive.
...
\item We assemble a \emph{unified mathematical framework} (\Cref{sec:framework}), a
total representation category $\int\Repphys$ with a global realization functor,
...
discharges, and (iii) the single \emph{emergent representational property} that
appears only in the composite.  The
...
Part~\PIV\ supplies (co)homology, characteristic classes, bordism, and, in the
...
(\PIV/T2), which classifies extended TQFTs by their value on a point, announces the
...
in which many physical representatives realize one logical/observable content, robustly
...
collapsing the domain index, which is exactly the modular discipline of \Cref{sec:intro}
...
one operation appearing in three forms.  The conjectural equality of the motivic coaction tree
...
(\PVI/T3).  Contractible error strings are boundaries and act trivially, which is exactly the
...
We now state and assemble the main result of the synthesis.  It has three faces: a
...
The status system records how much epistemic warrant each entry carries; here we make it
...
\Cref{tab:census} brings out two contrasts.  Part~\PIV\ (algebraic topology)
...
functor as a physical theory) are heuristic by construction.  That fits the module's
character: the abstract compositional grammar is where physical interpretation is most
optional.
