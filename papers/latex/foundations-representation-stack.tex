...
representation $P$, a translation datum $\tau$, and an
...
gauge equivalence. Structure becomes number through the
...
The ladder is, by design, \emph{circular}.
Part~I \emph{posits} that representation entries behave like a stack: local
data glue not on the nose but up to equivalence. Part~VI \emph{proves} what a
...
We now give the four central definitions of the formalism. They are stated
...
The upshot of Part~I is that Parts~II--VI are, formally, successive
...
explicit realization pipeline, together with the status calculus of
\Cref{thm:status}, which is original to this program.
